---
title: VS-02.0 Data Standardization and Integration — Business Layer
abstract: "This document describes how **Raw Data** becomes **Standardized Data**: data from different sources is aligned to common structures, formats, classifications, and meanings — without changing a single value — so datasets that looked different at the source can be compared and combined. It covers the complete lifecycle in four process groups: _Design_ (define the target structures and mapping rules), _Implementation_ (build the mapping and validation pipeline), _Execution_ (standardize each incoming dataset), and _Management_ (track mapping coverage and improve). It is intended as a canvas for National Statistical Offices (NSOs) to adapt to their own context."
